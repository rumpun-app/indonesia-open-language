...
Contributions are proposals, not canonical language data. A contributor creates a structured change set with a reason and optional evidence. The proposal begins as `draft` and moves to `submitted` only when the contributor explicitly submits it.
